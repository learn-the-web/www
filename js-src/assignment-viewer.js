(function () {
  ...
  var $gradingTypeWrap = document.querySelector(
    ".assignment-grading-type-wrap"
  );
  ...
  var $btnDownloadWrap = document.querySelector(
    ".assignment-download-btn-wrap"
  );
  ...
  var $deliverablesDefault = document.querySelector(
    ".assignment-deliverables-default"
  );
  var $deliverablesHeading = document.querySelector(
    ".assignment-deliverables-heading"
  );
  ...
  var labels = {
    code: document
      .querySelector('[property="assignment-code"]')
      .getAttribute("content"),
    fork: document
      .querySelector('[property="assignment-fork"]')
      .getAttribute("content"),
    "download-markbot": document
      .querySelector('[property="assignment-download-markbot"]')
      .getAttribute("content"),
    online: document
      .querySelector('[property="assignment-online"]')
      .getAttribute("content"),
    ghissue: document
      .querySelector('[property="assignment-ghissue"]')
      .getAttribute("content"),
    show: document
      .querySelector('[property="assignment-show"]')
      .getAttribute("content"),
  };
  ...
  var populateInitialDetails = function (details) {
    ...
    if (
      dueDates &&
      typeof dueDates === "object" &&
      courseSection &&
      courseSection.indexOf(details.course) > -1
    ) {
      var section = courseSection
        .replace(details.course, "")
        .replace(/[^\d]*/, "");

      if (section && dueDates[section]) {
        var weekStart = getDateOfISOWeek(
          parseInt(dueDates[section].substr(6, 2), 10),
          parseInt(dueDates[section].substr(0, 4), 10)
        );
        var dueTime = new Date(
          weekStart.getFullYear(),
          weekStart.getMonth(),
          weekStart.getDate() +
            parseInt(dueDates[section].substr(9, 1), 10) -
            1,
          dueDates[section].substr(11, 2),
          dueDates[section].substr(14, 2)
        );
  ...
  var getScreenshotsUrl = function (href) {
    return (
      href.replace("github.com", "api.github.com/repos") +
  ...
  var formatMarkdown = function (href, markdown) {
    ...
    markdown = markdown.replace(
      '<img src="',
      '<img class="img-flex" src="' + href + "/raw/gh-pages/"
    );
    // Add wrapper around tables
    markdown = markdown.replace(
      /<table/g,
      '<div class="assignment-viewer-table-wrapper scrollable"><table'
    );
  ...
  var convertTaskLists = function () {
    ...
    [].forEach.call(lists, function (list, listIndex) {
      [].forEach.call(list.children, function (li, liIndex) {
        var newLabel,
          newInput,
          id = "assignment-" + listIndex + "-" + liIndex;
  ...
  var populateViewer = function (readme, screenshots) {
    ...
    if (readme.submit && $btn.dataset.submissionType == "online")
      $btn.href = readme.submit;
    if (readme.download && $btn.dataset.submissionType == "download-markbot")
      $btn.href = readme.download;
    if (readme.download && $btn.dataset.submissionType !== "download-markbot")
      showDownloadButton(readme.download);
          ...
          responses[1].text().then(function (screenshotsData) {
            ...
            var screenshots =
              responses[1].status >= 200 && responses[1].status <= 299
                ? parseScreenshots(screenshotsData)
                : false;
  ...
  var tiggerViewerOpen = function () {
    ...
    url = elem.href;
    submissionType = elem
      .querySelector('meta[property="submission-type"]')
      .getAttribute("content");
    ...
    populateInitialDetails({
      ...
      course: elem
        .querySelector('meta[property="course"]')
        .getAttribute("content"),
      due: elem.querySelector('meta[property="due"]').getAttribute("content"),
      dueDates: elem.querySelector('script[class="assessment-due-dates"]')
        .innerHTML,
      worth: elem
        .querySelector('meta[property="worth"]')
        .getAttribute("content"),
      gradingType: elem
        .querySelector('meta[property="grading-type"]')
        .getAttribute("content"),
      satisfies:
        "CLRs " +
        elem.querySelector('meta[property="clr"]').getAttribute("content"),
    });
  ...
  window.addEventListener("hashchange", function (e) {
    if (
      allAssignmentIds.indexOf(window.location.hash) > -1 ||
      window.location.hash === "#" ||
      !window.location.hash
    ) {
      e.preventDefault();
  ...
  [].forEach.call(
    document.querySelectorAll('a[data-control="assignment-viewer"]'),
    function (elem) {
      ...
      elem.addEventListener("click", function (e) {
        ...
        if (
          window.location.hash.replace(/#/g, "") == this.id.replace(/#/g, "")
        ) {
          tiggerViewerOpen();
        } else {
  ...
  document
    .getElementById("assignment-close-btn")
